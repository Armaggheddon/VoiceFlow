...
class VoiceFlowClient:
    """Synchronous client for VoiceFlow API."""
    ...
    def __init__(
        ...
        """
        Initialize the VoiceFlow client.
    ...
    def transcribe(
        ...
        task_id = self._submit_transcription(audio_file, filename)
        
        # Poll for result
        return self._poll_transcription_result(task_id, poll_timeout)
    ...
    def synthesize(
        ...
        task_id = self._submit_synthesis(text)
        
        # Poll for result
        result = self._poll_synthesis_result(task_id, poll_timeout)
        
        # Process audio based on output format
        if output_format == AudioOutputFormat.URL:
            return result
        elif output_format == AudioOutputFormat.NUMPY:
            return self._convert_to_numpy(result)
        elif output_format == AudioOutputFormat.FILE:
            return self._save_to_file(result, save_path)
        else:
            raise VoiceFlowError(f"Unsupported output format: {output_format}")
    
    def get_task_result(self, task_id: str) -> Union[TranscriptionResult, SynthesisResult]:
        """
    ...
    def _submit_transcription(self, audio_file: Union[str, Path, bytes], filename: Optional[str]) -> str:
        """Submit a transcription task."""
        try:
            if isinstance(audio_file, (str, Path)):
                with open(audio_file, "rb") as f:
                    files = {"audio_file": (Path(audio_file).name, f, "audio/wav")}
                    response = self._client.post(f"{self.base_url}/v1/transcribe", files=files)
            else:
                # Handle bytes
    ...
    def _submit_synthesis(self, text: str) -> str:
        """Submit a synthesis task."""
        try:
            data = {"text": text}
            response = self._client.post(f"{self.base_url}/v1/synthesize", data=data)
            response.raise_for_status()
            result = response.json()
    ...
    def _poll_transcription_result(self, task_id: str, timeout: float) -> TranscriptionResult:
    ...
    def _poll_synthesis_result(self, task_id: str, timeout: float) -> SynthesisResult:
    ...
    def _convert_to_numpy(self, result: SynthesisResult) -> SynthesisResult:
        ...
        try:
            ...
            audio_bytes = download_audio_as_bytes(result.audio_url, timeout=self.timeout)
    ...
    def _save_to_file(self, result: SynthesisResult, save_path: Optional[Union[str, Path]]) -> SynthesisResult:
